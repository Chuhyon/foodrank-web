...
from .food_filter import is_menu_keyword, is_brand_keyword, MENU_KEYWORDS, BRAND_KEYWORDS, extract_food_term
...
def _extract_unique_food_terms(phrases: list, filter_fn) -> list:
    """
    ...
    for phrase in phrases:
        # 구절 자체가 음식 키워드 단일어면 그대로 사용
        term = phrase if filter_fn(phrase) and len(phrase) <= 8 else extract_food_term(phrase)
...
def _get_trending_keywords(filter_fn) -> list:
    """
    한국 급상승 검색어 → 핵심 음식 키워드 반환
    없으면 시드 키워드의 급상승 관련어로 fallback
    """
    pytrends = _build_pytrends()

    # 1차: 실시간 급상승 검색어 (realtime 엔드포인트 우선)
    for method in ["realtime", "daily"]:
        try:
            ...
            found = _extract_unique_food_terms(phrases, filter_fn)
            if found:
    ...
    try:
        ...
        found = _extract_unique_food_terms(rising_phrases, filter_fn)
        if found:
